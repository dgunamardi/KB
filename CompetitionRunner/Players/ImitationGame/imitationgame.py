...
def parse_coord(s):
    if s == 'pass':
        return None
    return W+1 + (N - int(s[1:])) * W + colalpha.index(s[0].upper())

def str_coord(c):
    if c is None:
        return 'p'
    row, col = divmod(c - (W+1), W)
    return '%c%d' % (colalpha[col], N - row)
...
def play_chess_go(computer_black=False):
    if(sys.argv[1] == "err0" or sys.argv[1] == "err1" or sys.argv[1] == "err2"):
        ...
        for i in range (len(arrmap)):
            if arrmap[i] == '.':
                container.append(i)
        randomindex = random.randint(0,len(container))
        print(str_coord(container[randomindex]))
        file.close()
    else:
        file = open("map.txt","w")
