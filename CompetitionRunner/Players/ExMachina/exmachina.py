...
W = N + 2
empty = "\n".join([(N + 1) * ' '] + N * [' ' + N * '.'] + [(N + 2) * ' '])
colstr = 'ABCDEFGHI'  # string list for column. A ~ I for 9x9 board
# MAX_GAME_LEN = N * N
...
pat3src = [  # 3x3 playout pattern list; X,O are colors; x,o are their inverses
...
pat_gridcular_seq = [  # Sequence of coordinate offsets of progressively wider diameters in gridcular metric
    ...
    [[1, 5], [-1, 5], [1, -5], [-1, -5], [3, 4], [-3, 4], [3, -4], [-3, -4], [4, 3], [-4, 3], [4, -3], [-4, -3], [5, 1],
     [-5, 1], [5, -1], [-5, -1], ],
    [[0, 6], [0, -6], [2, 5], [-2, 5], [2, -5], [-2, -5], [4, 4], [-4, 4], [4, -4], [-4, -4], [5, 2], [-5, 2], [5, -2],
     [-5, -2], [6, 0], [-6, 0], ],
    [[1, 6], [-1, 6], [1, -6], [-1, -6], [3, 5], [-3, 5], [3, -5], [-3, -5], [5, 3], [-5, 3], [5, -3], [-5, -3], [6, 1],
     [-6, 1], [6, -1], [-6, -1], ],
    [[0, 7], [0, -7], [2, 6], [-2, 6], [2, -6], [-2, -6], [4, 5], [-4, 5], [4, -5], [-4, -5], [5, 4], [-5, 4], [5, -4],
     [-5, -4], [6, 2], [-6, 2], [6, -2], [-6, -2], [7, 0], [-7, 0], ],
]
spat_patterndict_file = 'patterns.spat'  # large-scale pattern file, you should download it manually first at http://pachi.or.cz/michi-pat/
large_patterns_file = 'patterns.prob'  # the same
...
def is_eyeish(board, c):  # detect if there there are an eye pattern
    eyecolor = None
    for d in neighbors(c):
        if board[d].isspace():
            continue
...
def is_eye(board, c):  # move when there are an eye pattern
...
def fix_atari(pos, c, singlept_ok=False, twolib_test=True, twolib_edgeonly=False):
    ...
    # In atari, it's enough if it's the opponent's group
    if pos.board[c] == 'x':
        return (True, [l])
    ...
    while True:
        ...
        if a and ccls:
            solutions += ccls
        # floodfill is better for big groups
    ...
    if l_new_2 is not None:
        # Check that we aren't caught in a ladder, if it's just two
...
# 3x3 pattern routines from pat3src above
...
def pat3_expand(pat):
    ...
    def pat_swapcolors(p):
        return [l.replace('X', 'Z').replace('x', 'z').replace('O', 'X').replace('o', 'x').replace('Z', 'O').replace('z',
                                                                                                                    'o')
                for l in p]
...
def go(arg):
    ...
    tree.expand()
    owner_map = W * W * [0]  # define map
    tree = tree_search(tree, N_SIMS, owner_map)  # search for best move
    ret = str_coord(tree.pos.last)  # return last tree position (final predicted best move)
    # if ret.len() < 2:
    #     print('pass')
    print('%s ' % ret)
    return ret
